...
## DONE: 基于human_files_short和dog_files_short
## 中的图像测试face_detector的表现
human_cv2_predict_human =  sum([1 for item in human_files_short if face_detector(item)])
dog_cv2_predict_human =  sum([1 for dog in dog_files_short if face_detector(dog)])
human_cv2_predict_human, dog_cv2_predict_human
...
# 
# __回答:__   
# 不合理，使用CNN来训练模型能在面部特征不清晰的情况下检测到人脸。
# 
# 
...
### DONE: 测试dog_detector函数在human_files_short和dog_files_short的表现
human_resNet_predict_dog =  sum([1 for item in human_files_short if dog_detector(item)])
dog_resNet_predict_dog =  sum([1 for dog in dog_files_short if dog_detector(dog)])
human_resNet_predict_dog, dog_resNet_predict_dog


# __回答:__   
#     
# - 使用 `dog_detector` 函数，`human_files_short`中图像检测到狗狗的占比0%
# - 使用 `dog_detector` 函数，`dog_files_short`中图像检测到狗狗的占比100%
...
### TODO: 设计你的算法
### 自由地使用所需的代码单元数吧
def ResNet50_predict_myImages(img_path):
    if face_detector(img_path):
        print("You look like a "+ Resnet50_predict_breed(img_path))
    elif dog_detector(img_path):
        print("It is a "+ Resnet50_predict_breed(img_path))
    else:
        print("There is no a dog or a face")
...
## 自由地使用所需的代码单元数吧
ResNet50_predict_myImages('myImages/keji.jpg')
ResNet50_predict_myImages('myImages/keji2.jpg')
ResNet50_predict_myImages('myImages/yingduan.jpg')
ResNet50_predict_myImages('myImages/yangzi.jpg')
ResNet50_predict_myImages('myImages/liuyifei.jpg')
ResNet50_predict_myImages('myImages/zhangyishan.jpg')
